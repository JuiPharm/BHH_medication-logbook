...
async function submitDispense(e) {
  ...
  if (!data.dispensedBy) {
    showToast('กรุณากรอกชื่อผู้จ่าย', 'warning');
    return;
  }
...
async function loadHistory() {
  if (!checkConfig()) return;
  const hn = (document.getElementById('historyHN')?.value || '').trim();
  if (!hn) {
    showToast('กรุณากรอก HN', 'warning');
    return;
  }
  ...
  let html = `
    ...
    <div class="table-responsive">
      <table class="table table-hover table-medicines">
        <thead>
          <tr>
            <th>วันที่</th><th>ชื่อยา</th><th>จำนวนที่จ่าย</th><th>คงเหลือ</th><th>ผู้จ่าย</th><th>ผู้รับ</th><th>Ward</th><th>หมายเหตุ</th>
          </tr>
        </thead><tbody>`;
  ...
  history.forEach(item => {
    html += `
      <tr>
        ...
        <td>${escapeHtml(item.DispensedBy || '-')}</td>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchInput) searchInput.addEventListener('keypress', e => { if (e.key === 'Enter') searchMedicine(); });

  const historyInput = document.getElementById('historyHN');
  if (historyInput) historyInput.addEventListener('keypress', e => { if (e.key === 'Enter') loadHistory(); });
